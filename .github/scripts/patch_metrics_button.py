...
frontend_path = Path('src/pages/admin/SubmissionsAdmin.tsx')
...
if old_button not in frontend:
    raise SystemExit('Could not find old metrics button')
frontend = frontend.replace(old_button, new_button, 1)
frontend_path.write_text(frontend)

backend_path = Path('server/src/index.js')
backend = backend_path.read_text()

start_marker = "app.post('/api/admin/submissions/:id/sync-metrics', verifyToken, requireAdmin, async (req, res) => {"
end_marker = "app.get('/api/admin/google-sheets-metrics', verifyToken, requireAdmin, async (_req, res) => {"
start = backend.find(start_marker)
end = backend.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit('Could not locate sync-metrics route')

new_route = r'''app.post('/api/admin/submissions/:id/sync-metrics', verifyToken, requireAdmin, async (req, res) => {
  try {
...
backend = backend[:start] + new_route + backend[end:]
